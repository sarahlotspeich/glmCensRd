...
#' @export
#'
glmCensRd <- function(params0, Y, X, W, D, Z = NULL, partX = 50, data, steptol = 1e-6, iterlim = 100) {
  suppressWarnings(
    mod <- nlm(f = loglik_normal_normal, p = params0, steptol = steptol, iterlim = iterlim, hessian = TRUE,
               Y = Y, X = X, D = D, W = W, Z = Z, partX = partX, data = data)
  )
  return(mod)
