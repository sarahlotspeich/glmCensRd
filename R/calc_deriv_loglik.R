...
#'
calc_deriv_loglik <- function(params, Y, X, W, D, Z = NULL, data, subdivisions = 100, distY = "normal", distX = "normal") {
  p <- length(params)
  eps <- params * (10 ^ (- 4))
  ...
  for (j in 1:p) {
    ...
    l0 <- calc_indiv_loglik(params = params0, Y = Y, X = X, W = W, D = D, Z = Z, subdivisions = subdivisions, distY = distY, distX = distX, data = data)

    # params + eps
    params1 <- matrix(data = params + eps * ej, nrow = p, ncol = 1)
    l1 <- calc_indiv_loglik(params = params1, Y = Y, X = X, W = W, D = D, Z = Z, subdivisions = subdivisions, distY = distY, distX = distX, data = data)
