...
#' @param Wl name of lower bound of interval censoring \code{X}.
#' @param Wu name of upper bound of interval censoring \code{X}.
...
#' @param data dataframe containing at least columns \code{Y}, \code{X}, \code{Wl}, \code{Wu}, \code{D}, \code{Z}.
...
#'
loglik_int = function(params, Y, X, Wl, Wu,D, Z = NULL, data, distY = "normal", distX = "normal", subdivisions = 100) {
  ####################################################
  # Pre-processing ###################################
  ####################################################
  ...
  if (nrow(cens_data) > 0) {
    integrate_pYXandZ = function(data_row) {
      Wli = as.numeric(data_row[Wl])
      Wui = as.numeric(data_row[Wu])
      Yi = data_row[Y]
      Zi = data_row[Z]
      return(
